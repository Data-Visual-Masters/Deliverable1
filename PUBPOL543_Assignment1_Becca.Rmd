...
```{r}
big=big[big$County%in%c("King","Snohomish","Pierce","Clark","Thurston","Kitsap","Whatcom","Spokane","Benton","Island","Skagit"),]
```
...
###
plot4 = plot3 + geom_text(vjust=-5, #hjust if flipping
                          size = 2,
                          aes(y = Percent ,
                              label = LABELS))
plot4# + coord_flip() # wanna flip the plot?
...
(CountyOrd=tableFreq[order(tableFreq$County),'County'])
```
